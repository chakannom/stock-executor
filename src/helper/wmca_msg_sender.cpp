...
{
    web::json::value requestModel = web::json::value::parse(m_strData);
    CStringA strId(requestModel.at(L"id").as_string().c_str());
    CStringA strPw(requestModel.at(L"pw").as_string().c_str());
    CStringA strCertPw(requestModel.at(L"certPw").as_string().c_str());
...
void CWmcaMsgSender::InquireCurrentPrice(HWND hWnd)
{
    web::json::value requestModel = web::json::value::parse(m_strData);
    //종목코드를 가져옵니다
    CStringA strCode(requestModel.at(L"code").as_string().c_str());
